...
#
# apps = Flask(__name__)
# CORS(apps, origins="http://localhost:3000", supports_credentials=True)
#
# # Endpoint untuk mengambil berita dan melakukan analisis sentimen
# @apps.route("/analyze", methods=["POST"])
...
#         }), 500
#
# @apps.route('/change-sentiment', methods=['POST'])
...
#         return jsonify({"error": str(e)}), 500
#
# @apps.route("/sentimen", methods=['GET'])
# def get_sentimen():
#     try:
#
...
#         return jsonify({"error": str(e)}), 500
#
# @apps.route("/sentimen-bulanan", methods = ['GET'])
...
#         return jsonify({"error": str(e)}), 500
#
# @apps.route("/berita", methods = ['POST'])
# def add_berita():
#     try:
#         data = request.json
